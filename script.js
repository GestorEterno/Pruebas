...
class ChannelsCarousel {
    ...
    updateCarousel() {
        ...
        const containerWidth = container ? container.clientWidth - 120 : 1080;
        
        // Calcular ancho de cada item
        const itemWidth = (containerWidth - (this.itemsPerView - 1) * this.gap) / this.itemsPerView;
        
        // CORRECCIÓN CRÍTICA: Calcular posición de página actual
        const currentPage = Math.floor(this.currentPosition / this.itemsPerView);
        
        // CORRECCIÓN: Calcular desplazamiento EXACTO por página
        // Solo multiplicar por itemsPerView cuando currentPosition está en modo página
        const translateX = -(currentPage * (itemWidth + this.gap) * this.itemsPerView);
        
        // CORRECCIÓN: Actualizar maxPosition basado en páginas
        const totalPages = Math.ceil(this.totalItems / this.itemsPerView);
        this.maxPosition = Math.max(0, totalPages - 1);
    ...
    move(direction) {
        // CORRECCIÓN: Mover por páginas completas, no por items individuales
        const currentPage = Math.floor(this.currentPosition / this.itemsPerView);
        const newPage = currentPage + direction;
        
        if (newPage >= 0 && newPage <= this.maxPosition) {
            this.currentPosition = newPage * this.itemsPerView;
            this.updateCarousel();
        }
    }
    ...
    goToPage(page) {
        // CORRECCIÓN: Validar que la página esté dentro del rango
        if (page >= 0 && page <= this.maxPosition) {
            this.currentPosition = page * this.itemsPerView;
            this.updateCarousel();
        }
    }
