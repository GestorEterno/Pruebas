...
class ShortsCarousel {
    ...
    updateCarousel() {
        ...
        // DESPLAZAMIENTO CORREGIDO - SIN DUPLICAR itemsPerView
    ...
    move(direction) {
        // Mover exactamente itemsPerView elementos (una página completa)
        const newPosition = this.currentPosition + (direction * this.itemsPerView);
...
class ChannelsCarousel {
    ...
    regenerateIndicators(totalPages) {
        ...
        if (this.indicators[Math.floor(this.currentPosition / this.itemsPerView)]) {
            this.indicators[Math.floor(this.currentPosition / this.itemsPerView)].classList.add('active');
        }
    }
    ...
    addTouchSupport() {
        ...
        this.track.addEventListener('touchend', () => {
            ...
            if (Math.abs(diff) > threshold) {
                if (diff > 0) {
                    this.move(-1); // Deslizar izquierda → anterior
                } else {
                    this.move(1); // Deslizar derecha → siguiente
                }
            }
        });
    }
