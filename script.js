...
class ShortsCarousel {
    ...
    updateIndicators() {
        ...
        // Calcular cuántos indicadores necesitamos
        const totalPages = Math.ceil(this.totalItems / this.itemsPerView);
        
        // Si la cantidad de indicadores no coincide, regenerarlos
        if (this.indicators.length !== totalPages) {
            this.regenerateIndicators(totalPages);
        }
        
        // Calcular qué página está activa
        const activePage = Math.floor(this.currentPosition / this.itemsPerView);
    ...
    regenerateIndicators(totalPages) {
        const indicatorsContainer = document.querySelector('.shorts-indicators');
    ...
    updateCarousel() {
        ...
        const containerWidth = container ? container.clientWidth - 120 : 1080; // 120px para botones
        
        // CORRECCIÓN: Calcular maxPosition basado en páginas, no en items
        const totalPages = Math.ceil(this.totalItems / this.itemsPerView);
        this.maxPosition = Math.max(0, totalPages - 1);
        
        // Calcular ancho de cada item
        const itemWidth = (containerWidth - (this.itemsPerView - 1) * this.gap) / this.itemsPerView;
        
        // CORRECCIÓN: Mover por páginas, no por items individuales
        const translateX = -(this.currentPosition * (itemWidth + this.gap) * this.itemsPerView);
        
        this.track.style.transform = `translateX(${translateX}px)`;
        this.updateIndicators();  // Actualizar puntos
        this.updateButtons();
    }
    ...
    goToPage(page) {
        const newPosition = page * this.itemsPerView;
        if (newPosition >= 0 && newPosition <= this.totalItems - this.itemsPerView) {
            this.currentPosition = newPosition;
            this.updateCarousel();
        }
    }
...
// CARRUSEL DE CANALES - VERSIÓN CORREGIDA (MISMA LÓGICA QUE SHORTS)
class ChannelsCarousel {
    constructor() {
        ...
        this.maxPosition = Math.max(0, this.totalPages - 1);
    ...
    calculateItemsPerView() {
        const width = window.innerWidth;
        if (width >= 1024) return 5;    // Desktop: 5 canales
        if (width >= 768) return 2;     // Tablet: 2 canales
        return 1;                       // Móvil: 1 canal
    }
    ...
    updateItemsPerView() {
        ...
        // Ajustar currentPosition basado en la nueva cantidad de elementos por vista
    ...
    updateCarousel() {
        ...
        // Calcular ancho del contenedor disponible
        ...
        // **CORRECCIÓN CRÍTICA:** Calcular desplazamiento por PÁGINAS, no por elementos individuales
        // Cada página mueve itemsPerView elementos a la vez
        const translateX = -(this.currentPosition * (itemWidth + this.gap) * this.itemsPerView);
    ...
    updateIndicators() {
        ...
        // Calcular cuántos indicadores necesitamos (uno por página)
        const totalPages = Math.ceil(this.totalItems / this.itemsPerView);
        ...
        // Calcular qué página está activa
        const activePage = this.currentPosition;
    ...
    regenerateIndicators(totalPages) {
        const indicatorsContainer = document.querySelector('.channels .carousel-indicators');
        if (!indicatorsContainer) return;
        ...
        // Aplicar clase active al indicador actual
        if (this.indicators[this.currentPosition]) {
            this.indicators[this.currentPosition].classList.add('active');
        }
    }
    ...
    updateButtons() {
        ...
        if (this.nextBtn) {
            const isDisabled = this.currentPosition >= this.maxPosition;
    ...
    goToPage(page) {
        if (page >= 0 && page <= this.maxPosition) {
    ...
    addTouchSupport() {
        ...
        this.track.addEventListener('touchend', () => {
            ...
            if (Math.abs(diff) > threshold) {
                if (diff > 0) {
                    this.move(1); // Deslizar izquierda → siguiente
                } else {
                    this.move(-1); // Deslizar derecha → anterior
                }
            }
        });
    }
